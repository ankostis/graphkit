...
:attr:`Network.graph`
    ...
    :class:`Operation` and :class:`_DataNone` nodes.
...
class ExecutionPlan(
    ...
    def _can_evict_value(self, name):
        """
        Determines if a _DataNone is ready to be evicted from solution.
    ...
    def add_op(self, operation):
        ...
        for n in operation.needs:
            ...
            if isinstance(n, sideffect):
                kw["sideffect"] = True
            self.graph.add_edge(_DataNone(n), operation, **kw)
        ...
        for p in operation.provides:
            ...
            if isinstance(n, sideffect):
                kw["sideffect"] = True
            self.graph.add_edge(operation, _DataNone(p), **kw)
            ...
            if isinstance(node, Operation):
                if not dag.adj[node]:
                    ...
                    if real_needs.issubset(op_satisfaction[node]):
                        ...
                        unsatisfied.append(node)
            elif isinstance(node, (_DataNone, str)):  # `str` are givens
                if node in ok_data:
